...
#align(
  left,
  rect(inset: 5pt)[Using identity below and $x ket(x')= x' ket(x'), med p ket(p') = p' ket(p); $ find $bra(x) [x,p] ket(alpha)$ in terms of  $psi_alpha (x) = braket(x,alpha)$$ 
...
#align(
  left,
  rect(inset: 5pt)[The ground state positiion space wavefunction of the Hydrogen atom is $ 
...
We first prove the following identity: $ 
     integral_RR_3 e^(-i  bold(q) dot  bold(x)) f(r) dif^3 r&=integral_0^infinity r^2  f(r) dif r integral_(0)^(pi) sin theta e^(-i q r cos theta)   dif theta integral_(0)^(2pi)  dif phi \ 
     &=_(u equiv cos theta) 2 pi integral_(0)^(infinity) r^2  f(r) dif r integral_(-1)^(1) e^(- i q r u)  dif u \ 
     & = 4 pi integral_(0)^(infinity) r^2  f(r) sin(q r)/(q r) dif r \ &= (4pi)/q integral_(0)^(infinity) f(r) sin(q r)r dif r
$ 
...
#align(
  left,
  rect(inset: 5pt)[Find $expval((Delta x)^2 ) expectationvalue((Delta p)^2 )$ for a particle in a 1d infinite square well for the n-th eigenstate of potential, and find numerical result for ground and first state. ]
)
For odd n, we have $ 
...
frac(4a cos(beta a) , beta^2 ) =abs(frac(4 a^3 (-1)^n , n^2  pi^2 ))
$ and so $ 
